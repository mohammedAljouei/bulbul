...
interface LevelSelectorProps {
  value?: UserLevel;
  onChange: (level: UserLevel) => Promise<boolean>;
  onComplete: () => void;
}
...
export function LevelSelector({ value, onChange, onComplete }: LevelSelectorProps) {
  ...
  const handleLevelSelect = (level: UserLevel) => {
    ...
    onChange(level).catch(console.error);
  };
          ...
          <div className="space-y-4">
            {levels.map((level) => (
              ...
              >
                ...
                >
                  <level.icon className={`w-6 h-6 ${value === level.id ? 'text-white' : 'text-purple-600'}`} />
                </div>
                <div>
                  <h4 className="font-semibold text-lg mb-1">{level.title}</h4>
                  <p className={`text-sm ${value === level.id ? 'text-white/80' : 'text-purple-600'}`}>
                    {level.description}
                  </p>
                </div>
              </motion.button>
